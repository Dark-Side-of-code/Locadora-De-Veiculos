...
namespace Locadora_De_Veiculos.WindApp.Compartilhado
{
    public class ClassMaskValor
    {
        ...
        private static void Aplica_Leave_MascaraKm(object sender, EventArgs e)
        {
            TextBoxBase txt = (TextBoxBase)sender;
            valor = txt.Text.Replace("Km", "");
            txt.Text = string.Format("{0:C}", Convert.ToDouble(valor));
        }

        private static void Aplica_KeyUp_MascaraKm(object sender, KeyEventArgs e)
        {
            TextBoxBase txt = (TextBoxBase)sender;
            valor = txt.Text.Replace("Km", "").Replace(",", "").Replace(" ", "").Replace("00,", "");
            if (valor.Length == 0)
            {
                txt.Text = "0,00" + valor;
            }
        ...
        private static void Aplica_Leave_MascaraLitros(object sender, EventArgs e)
        {
            TextBoxBase txt = (TextBoxBase)sender;
            valor = txt.Text.Replace("L", "");
            txt.Text = string.Format("{0:C}", Convert.ToDouble(valor));
        }

        private static void Aplica_KeyUp_MascaraLitros(object sender, KeyEventArgs e)
        {
            TextBoxBase txt = (TextBoxBase)sender;
            valor = txt.Text.Replace("L", "").Replace(",", "").Replace(" ", "").Replace("00,", "");
            if (valor.Length == 0)
            {
                txt.Text = "0,00" + valor;
            }
        ...
        public static void AplicaMascaraKm(TextBoxBase txt)
        {
            txt.KeyPress += Aplica_KeyPress_Mascara;
            txt.Leave += Aplica_Leave_MascaraKm;
            txt.KeyUp += Aplica_KeyUp_MascaraKm;
        }
        public static void AplicaMascaraLitros(TextBoxBase txt)
        {
            txt.KeyPress += Aplica_KeyPress_Mascara;
            txt.Leave += Aplica_Leave_MascaraLitros;
            txt.KeyUp += Aplica_KeyUp_MascaraLitros;
        }
    }
}
